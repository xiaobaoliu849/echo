...
export function RealtimeTranscriptionPanel({ onComplete, onSwitchToLibrary, onOpenSettings }: Props) {
  ...
  return (
    <div className="vsRTPanel">
      ...
      <div className="vsRTToolbar">
        ...
        <div className="vsRTViewRow">
          <div className="vsRTViewSwitcher">
            ...
            <button type="button" onClick={() => setViewMode("transcript")} className={`vsRTViewBtn${viewMode === "transcript" ? " active" : ""}`}>
              📝 {t("文稿模式", "Transcript")}
            </button>
            <button type="button" onClick={() => setViewMode("timeline")} className={`vsRTViewBtn${viewMode === "timeline" ? " active" : ""}`}>
              ⏱️ {t("字幕时间轴", "Timeline Cues")}
            </button>
          </div>

          <div className="vsRTViewControls">
      ...
      {/* Main Workspace — layered card, metrics footer anchored */}
      <div className="vsRTWorkspace">
        {/* VIEW 1: Teleprompter Mode (上下平滑滚动与高亮聚焦) */}
        {viewMode === "teleprompter" && (
          <div ref={teleprompterBoxRef} className="vsRTWorkspaceScroll" style={{ padding: "28px 32px", display: "flex", flexDirection: "column", gap: "20px", scrollBehavior: "smooth" }}>
        ...
        {viewMode === "transcript" && (
          <div ref={transcriptBoxRef} className="vsRTWorkspaceScroll" style={{ padding: "24px 28px", fontSize: "16px", lineHeight: 1.8, color: "var(--text)", whiteSpace: "pre-wrap", wordBreak: "break-word" }}>
            {displayTranscript ? (
              <>
        ...
        {viewMode === "timeline" && (
          <div ref={timelineBoxRef} className="vsRTWorkspaceScroll" style={{ padding: "16px 20px", display: "flex", flexDirection: "column", gap: "10px" }}>
            {segments.length === 0 && !interim ? (
      ...
      <div className="vsRTActionBar">
        {phase === "done" ? (
          <div className="vsRTActionBar--done">
            {/* Save to Library */}
            <button onClick={handleSave} disabled={!displayTranscript.trim() || saving} className="vsBtnPrimary vsRTActionPrimary">
              {saving ? (
                <>
            ...
            {/* Export Dropdown */}
            <div className="vsRTDropdownWrap vsRTDropdownWrap--export">
              <button
                type="button"
                onClick={() => {
              ...
              {showExportMenu && (
                <div className="vsRTDropdownMenu">
                  <button
                    type="button"
                    onClick={() => handleExport("srt")}
                    className="vsRTDropdownItem"
                  >
            ...
            {/* Copy Dropdown */}
            <div className="vsRTDropdownWrap">
              <button
                type="button"
                onClick={() => {
