...
export async function streamCanvasGenerate(
  payload: any,
  handlers: StreamEventHandlers,
  options: { signal?: AbortSignal } = {}
): Promise<void> {
  const response = await apiFetch(`${API_BASE_URL}/api/canvas/generate`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Accept: 'text/event-stream'
  ...
  if (!response.body) {
    throw new Error('Canvas stream is unavailable: empty response body.');
  }
